...
print(f"Answer part 1: {len(imposible_beacon_indices)}")
...
for y in range(Y_MAX + 1):
    ...
    for ((sx, sy), sensor_reach_distance) in zip(sensor_indices, distances):
        ...
        if reach_distance_at_y := sensor_reach_distance - abs(y - sy):
    ...
    for x_start, x_end in interval_queue:
        ...
        if x < x_start:
            print(f"Answer part 2: Found beacon at {(x, y)} with the tuning frequency {4_000_000* x + y}.")
            found_beacon = True
            break
